...
public class Genetic {
	...
	**/
	public Solution beginSelfEvolution() {
		...
		initPopulation(); //creates random initial solutions //O(TMAX*E^2**MAXPOPULATION)
		updateBest();
		...
		try {
			while(timer <= cf.maxTime) {		
				...
				parents=parentSelection(cf.maxParents,cf.maxTournamentContenders); //O(MAX POPULATION)
				
				/* GENETIC OPERATORS */
				coin = randomValue.nextDouble();
				if(coin<=cf.pC) { //CROSSOVER
					children.addAll(applyCrossover(parents)); //O(MAX PARENTS * EXAMS)
				}
				
		
				coin = randomValue.nextDouble();
				if(coin<=cf.pM) { //MUTATION
					children.addAll(applySelfMutation(parents));  //O(MAX PARENTS)
				} 
				/* end GENETIC OPERATORS */
				
				fixPopulation(); //tries to fix infeasible solutions //O(TMAX*E^2**MAXPOPULATION)
//				computeFitness(); //already done by fix 
				...
				population = deleteDuplicates(population); //O(MAXPOP^2)
				...
				if(this.TSEra && System.currentTimeMillis() - timerStart < cf.maxTime - 55*1000)
				{
					...
					if(counterSA>cf.SAcounterActivation) {
						population.sort((p1,p2) -> p1.compareTo(p2));
						counterSA = 0;
						localSearchSA(iterationCounter,SA); //O(E^2)
					}
				}
					

				naturalSelection(children, children.size()); //O(MAXPOP*logMAXPOP)
				
				if(population.size()>cf.maxPopulation) //population size mismatch
					return this.globalBest; 
				
				updateBest();	//O(MAXPOPULATION +E)
